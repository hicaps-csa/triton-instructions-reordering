#!/bin/bash
# Correctness check for multi_consumer_cat_3kind_2d (Experiment A1).
# Confirms:
#   OFF: single 1-D tt.cat (fp32) feeds arith.fptosi, arith.truncf, and tt.fp_to_fp.
#   ON : original fp32 cat is dead; THREE parallel rewritten chains -- one
#        cat-on-i8, one cat-on-fp16, one cat-on-fp8e5 -- one per
#        (OperationName, ResultType) cache key. Worklist propagation also
#        sinks the reducers through the upstream reshapes.
...
for SHAPE in "${SHAPES[@]}"; do
  ...
  for PASS in OFF ON; do
    set_pass "$PASS"
    cache="/tmp/triton_cache_mcc3_${PASS,,}_${M}_${BM}x${BN}"
    ...
    if [ -n "$ttir" ]; then
      cp "$ttir" "$RESULTS_DIR/multi_consumer_cat_3kind_${PASS}_${M}_${BM}x${BN}.ttir"
    fi
  done
