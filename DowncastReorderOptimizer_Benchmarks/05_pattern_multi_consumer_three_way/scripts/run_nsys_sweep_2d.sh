#!/bin/bash
# nsys sweep: 3-kind multi-consumer cat (Experiment A1).
#   - 2 matrix sizes : 4096x4096 and 8192x8192
#   - 9 (m, n) tiles : {64, 128, 256} on each axis
#   - 2 pass states  : OFF, ON
# = 36 captures. Same structure as multi_consumer_cat_2d. The kernel has
# THREE distinct-kind consumers (fptosi-i8, truncf-fp16, fp_to_fp-fp8e5)
# fanning out from one fp32 tt.cat -- exercises the (OperationName, Type)
# dedup cache with 3 distinct keys.
...
RESULTS_DIR=/tmp/nsys_runs_mcc3
...
run_one() {
  ...
  local cache="/tmp/triton_cache_mcc3_${pass,,}"
  ...
  row=$(nsys stats --report cuda_gpu_kern_sum --format csv "$rep" 2>/dev/null \
        | grep multi_consumer_cat_3kind_2d_kernel | head -1)
  if [ -z "$row" ]; then
...
for PASS in OFF ON; do
  ...
  echo "==================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_mcc3_${PASS,,}"
  mkdir -p "/tmp/triton_cache_mcc3_${PASS,,}"
