"""Turn nsys_results_2d.csv (multi_consumer_cat_3kind_2d, Experiment A1)
into a human-readable report.

For each matrix size: 3x3 m-by-n tables (OFF us / ON us / speedup / BW) and
a per-tile detailed table. The kernel here is the 3-kind multi-consumer cat:
tt.cat (1-D fp32) feeds arith.fptosi (i8), arith.truncf (fp16), and
tt.fp_to_fp (fp8e5).

Bytes/launch:
    4*M*N (read a) + 4*M*N (read b)
  + 1*M*2*N (write i8,  size 2M x N)
  + 2*M*2*N (write fp16, size 2M x N)
  + 1*M*2*N (write fp8,  size 2M x N)
  = 4 + 4 + 2 + 4 + 2 = 16*M*N
"""
import csv
import os
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D multi_consumer_cat_3kind_2d_kernel -- nsys timing sweep\n")
        w("  Experiment A1: (OperationName, ResultType) dedup cache with 3\n")
        w("                 DISTINCT consumer kinds on a single fp32 tt.cat.\n")
        w("  Reducers: arith.fptosi (i8), arith.truncf (fp16), tt.fp_to_fp (fp8e5).\n")
        w("  Cache must produce 3 independent rewritten chains -- no false sharing.\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Block sizes : (BLOCK_M, BLOCK_N) in {64, 128, 256}^2\n")
        w("  Bytes/launch: 4*M*N (read a) + 4*M*N (read b)\n")
        w("              + 1*M*2*N (write i8) + 2*M*2*N (write fp16) + 1*M*2*N (write fp8)\n")
        w("              = 16*M*N\n\n")
        ...
        for size in sizes:
            ...
            w("  Pass ON  -- effective bandwidth (GB/s, 16*M*N bytes/launch)\n")
            render_matrix(f, DIMS, lambda m, n: (
