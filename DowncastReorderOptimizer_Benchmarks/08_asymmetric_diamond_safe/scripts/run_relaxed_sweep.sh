#!/bin/bash
# RELAXED-GATE sweep for asymmetric_diamond_trans_safe (trans -> {truncf, store}).
# Patches the diamond-bail in DowncastReorderOptimizer.cpp, rebuilds, sweeps with relaxed
# gate, then restores. Trap-protected restore on any exit so the working tree
# stays clean.
#
# NOTE vs the earlier run_relaxed_sweep.sh on the mulf variant: that one had a
# bug where `cp -p` restore kept the backup's mtime, so the post-restore
# ninja said "no work to do" and the loaded .so retained the relaxed code.
# Fixed here by `touch`-ing the file before rebuild.
...
RESULTS_DIR=/tmp/nsys_runs_adts_relaxed
NCU_RUN_DIR=/tmp/ncu_runs_adts_relaxed
...
restore_source() {
  if [ -f "$BACKUP" ]; then
    ...
    touch "$PASS_SRC"  # force ninja to see the change
    rm -f "$BACKUP"
...
if ! grep -q "RELAXED_DIAMOND_GATE" "$PASS_SRC"; then
  echo ">> patch FAILED to apply; trap will restore"; exit 1
fi
...
run_one_nsys() {
  ...
  local cache="/tmp/triton_cache_adts_relaxed_${pass,,}"
  ...
  if [ -z "$row" ]; then
    echo "$pass,$size,$size,$bm,$bn,$lin,NOSTATS,NOSTATS,NOSTATS,NOSTATS,NOSTATS,NOSTATS,NOSTATS" >> "$CSV"; return
  fi
...
echo "==================================================================="
echo "==  RELAXED nsys sweep (pass ON; gate relaxed -> diamond rewrite  =="
echo "==  fires). Compare vs standard nsys_results_2d.                  =="
echo "==================================================================="
for PASS in OFF ON; do
  if [ "$PASS" = "OFF" ]; then set_pass_off; else set_pass_on; fi
  grep -n "add_downcast_reorder_optimizer" "$COMPILER"
  rm -rf "/tmp/triton_cache_adts_relaxed_${PASS,,}"; mkdir -p "/tmp/triton_cache_adts_relaxed_${PASS,,}"
...
for ENTRY in "${NCU_CONFIGS[@]}"; do
  ...
  REP="$NCU_RUN_DIR/$TAG.ncu-rep"; CSV2="$NCU_RUN_DIR/$TAG.csv"; LOG="$NCU_RUN_DIR/$TAG.log"
  echo "  -> $TAG"
