"""Turn nsys_results_2d.csv (asymmetric_diamond_trans_safe_2d sweep) into a human-readable report.

For each matrix size, render 3x3 m-by-n tables (OFF us / ON us / speedup /
BW) and a per-tile detailed table. The kernel here is the safe-ops
asymmetric diamond on tt.trans: trans (fp32) feeds both truncf (fp16
store) and a direct fp32 store (Triton elided the reshape no-ops on the
wide leg, leaving the minimal diamond shape).
"""
import csv
import os
...
def bw_gb_s(M: int, N: int, ns: float) -> float:
    """Bytes/launch:
        4*M*N (read input fp32)
      + 2*M*N (write fp16) + 4*M*N (write fp32)
      = 10*M*N
    """
    if ns <= 0:
        return 0.0
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D asymmetric_diamond_trans_safe_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer diamond gate (strict bail on non-reducer user)\n")
        w("  Diamond head : tt.trans (fp32) -> { truncf -> fp16 store, direct fp32 store }\n")
        w("                 (minimal diamond -- no arith on wide leg)\n")
        w("  Expected     : OFF and ON identical (gate bails); see run_relaxed_sweep.sh\n")
        w("                 for the relaxed-gate variant that quantifies the avoided regression.\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Bytes/launch: 4*M*N (read fp32) + 2*M*N (write fp16) + 4*M*N (write fp32)\n")
        w("              = 10*M*N\n\n")
        ...
        for size in sizes:
            ...
            w("  Pass ON  -- effective bandwidth (GB/s, 18*M*N bytes/launch)\n")
            render_matrix(f, DIMS, lambda m, n: (
