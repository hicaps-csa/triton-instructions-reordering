"""Aggregate ncu CSVs from the 2-D asymmetric-diamond-trans-safe sweep into a focused report."""
import csv
from pathlib import Path

RUN_DIR = Path("/tmp/ncu_runs_adts")
...
def main():
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  ncu detailed profiling -- 2-D asymmetric_diamond_trans_safe_2d_kernel @ M = N = 8192\n")
        ...
        w("  Note: with the current strict diamond gate, OFF and ON IR are identical;\n")
        w("        expect within-noise duration deltas. See run_relaxed_sweep.sh for\n")
        w("        the relaxed-gate variant that quantifies the avoided regression.\n")
        w("=" * 96 + "\n\n")
