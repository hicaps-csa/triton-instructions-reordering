"""Turn nsys_results_2d.csv (multi_consumer_trans_trunc_2d sweep) into a human-readable report.

For each matrix size, render 3x3 m-by-n tables (OFF us / ON us / speedup /
BW) and a per-tile detailed table. The kernel here is multi-consumer trans:
tt.trans (fp32) feeds both arith.fptosi (i8 store) and arith.truncf (fp16 store).
"""
import csv
import os
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D multi_consumer_trans_trunc_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer dedup cache (multi-reducer-kind on tt.trans)\n")
        w("  Shared mover : tt.trans (fp32) -> { fptosi -> i8 store, truncf -> fp16 store }\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Bytes/launch: 4*M*N (read fp32) + 1*M*N (write i8) + 2*M*N (write fp16)\n")
        w("              = 7*M*N\n\n")
