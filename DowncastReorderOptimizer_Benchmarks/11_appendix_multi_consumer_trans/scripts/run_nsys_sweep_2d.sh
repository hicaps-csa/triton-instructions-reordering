#!/bin/bash
# nsys sweep: 2-D multi-consumer trans kernel (trans -> {fptosi-i8, truncf-fp16})
#   - 2 matrix sizes : 4096x4096 and 8192x8192
#   - 9 (m, n) tiles : {64, 128, 256} on each axis
#   - 2 pass states  : OFF, ON
# = 36 captures. Exercises the (OperationName, Type)-keyed dedup cache in
# applyMultiConsumerOptimization on tt.trans (Exp 2 was single-consumer null;
# this asks whether the multi-reducer-kind story differs on the same mover).
...
RESULTS_DIR=/tmp/nsys_runs_mctt
...
run_one() {
  ...
  local cache="/tmp/triton_cache_mctt_${pass,,}"
  ...
  row=$(nsys stats --report cuda_gpu_kern_sum --format csv "$rep" 2>/dev/null \
        | grep multi_consumer_trans_trunc_2d_kernel | head -1)
  if [ -z "$row" ]; then
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_mctt_${PASS,,}"
  mkdir -p "/tmp/triton_cache_mctt_${PASS,,}"
