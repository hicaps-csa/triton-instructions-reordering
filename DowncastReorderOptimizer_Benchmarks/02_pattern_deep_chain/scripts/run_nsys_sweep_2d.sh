#!/bin/bash
# nsys sweep: 2-D deep-chain kernel
#   - 2 matrix sizes : 4096x4096 and 8192x8192
#   - 9 (m, n) tiles : {64, 128, 256} on each axis
#   - 2 pass states  : OFF, ON
# = 36 captures. The kernel exercises the 4-deep mover chain
# (reshape -> reshape -> cat -> reshape -> trans) before the trunc.
...
RESULTS_DIR=/tmp/nsys_runs_deep_chain
...
run_one() {
  ...
  local cache="/tmp/triton_cache_deepchain_${pass,,}"
  ...
  row=$(nsys stats --report cuda_gpu_kern_sum --format csv "$rep" 2>/dev/null \
        | grep deep_chain_2d_kernel | head -1)
  if [ -z "$row" ]; then
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_deepchain_${PASS,,}"
  mkdir -p "/tmp/triton_cache_deepchain_${PASS,,}"
