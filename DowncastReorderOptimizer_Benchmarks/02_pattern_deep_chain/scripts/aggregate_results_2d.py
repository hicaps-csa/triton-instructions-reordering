"""Turn nsys_results_2d.csv (deep_chain_2d sweep) into a human-readable report.

For each matrix size, render 3x3 m-by-n tables (OFF us / ON us / speedup /
BW) and a per-tile detailed table. The kernel here is the 4-deep mover
chain (reshape -> reshape -> cat -> reshape -> trans -> fptosi).
"""
import csv
import os
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D deep_chain_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer (sink fp32->i8 trunc past a 4-deep mover chain)\n")
        w("  Chain: reshape -> reshape -> cat -> reshape -> trans -> fptosi\n")
        w("=" * 96 + "\n\n")
