"""Aggregate nsys_results_16384.csv into a focused report (join-quant 2D @ 16384^2)."""
import csv
import os

CSV_PATH = "$PATTERN_DIR/nsys_results_16384.csv"
TXT_PATH = "$PATTERN_DIR/nsys_results_16384.txt"
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D join_quant_2d_kernel @ 16384^2 -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer (push fp32->i8 truncation before tt.join)\n")
        w("=" * 96 + "\n\n")

        w("Setup\n")
        w("-" * 96 + "\n")
        w("  GPU         : RTX 2080 Ti (68 SMs, 11 GiB)\n")
        w("  Stack       : Triton 3.6.0 / nsys 2023.2.3 / torch 2.9.1+cu128\n")
        w("  Iterations  : 50 between cudaProfilerStart/Stop, +3 warmup\n")
        w("                (halved vs 8192 sweep -- each launch is ~4x heavier)\n")
        w("  Reported    : per-launch avg from `nsys cuda_gpu_kern_sum`\n")
        w("  Block sizes : (BLOCK_M, BLOCK_N) in {64, 128, 256}^2 -- 9 distinct kernels\n")
        w("  Bytes/launch: 4*M*N (read a) + 4*M*N (read b) + 1*M*2*N (write i8) = 10*M*N\n")
        w("              = 2.5 GiB at M = N = 16384\n\n")
        ...
        for size in sizes:
            ...
            render_matrix(f, DIMS, lambda m, n: (
                f"{idx[('OFF', size, m, n)]['avg_ns']/1000:>9.1f}"
                if ('OFF', size, m, n) in idx else "      n/a"))
            w("\n")

            w("  Pass ON  -- avg per-launch time (us)\n")
            render_matrix(f, DIMS, lambda m, n: (
                f"{idx[('ON', size, m, n)]['avg_ns']/1000:>9.1f}"
            ...
            w("  Pass ON  -- effective bandwidth (GB/s, 10*M*N bytes/launch)\n")
            render_matrix(f, DIMS, lambda m, n: (
                f"{bw_gb_s(size, size, idx[('ON', size, m, n)]['avg_ns']):>9.1f}"
                if ('ON', size, m, n) in idx else "      n/a"))
            w("\n")

            w("  Detailed per-tile table\n")
            w(f"  {'(m,n)':>10} | {'tile':>7} | {'grid':>14} | "
              f"{'OFF us':>10} | {'ON us':>10} | {'speedup':>8} | "
              f"{'BW ON GB/s':>10}\n")
            w("  " + "-" * 95 + "\n")
            for m in DIMS:
                for n in DIMS:
                    ...
                    w(f"  {f'{m}x{n}':>10} | {tile:>7} | "
                      f"{f'{gx}x{gy}={grid}':>14} | "
                      f"{off['avg_ns']/1000:>10.2f} | "
                      f"{on['avg_ns']/1000:>10.2f} | "
        ...
        w("Comparison vs 8192^2 results (from nsys_results_2d.txt)\n")
        w("-" * 96 + "\n")
        w("  At 8192^2, the only meaningful win was at (256, 256): 1.11x\n")
        w("  All other tile shapes were essentially flat between OFF and ON.\n")
        w("  16384^2 quadruples the launch work; relative speedups can change\n")
        w("  if a tile that was below the spill cliff at 8192 crosses it at 16384.\n\n")
