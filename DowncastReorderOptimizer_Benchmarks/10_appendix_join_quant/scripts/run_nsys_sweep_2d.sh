...
RESULTS_DIR=/tmp/nsys_runs_join_quant_2d
...
run_one() {
  ...
  local cache="/tmp/triton_cache_join_quant_2d_${pass,,}"
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_join_quant_2d_${PASS,,}"
  mkdir -p "/tmp/triton_cache_join_quant_2d_${PASS,,}"
