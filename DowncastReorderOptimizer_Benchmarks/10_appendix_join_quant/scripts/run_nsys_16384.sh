#!/bin/bash
# Focused nsys sweep at 16384^2 for the join-quant kernel.
# 18 captures: {OFF, ON} x {64, 128, 256}^2 tiles.
# Writes a new CSV (nsys_results_16384.csv) so the existing 4096/8192 data
# isn't disturbed.
...
KERNEL_NAME=join_quant_2d_kernel
RESULTS_DIR=/tmp/nsys_runs_join_16384
CSV=$PATTERN_DIR/nsys/nsys_results_16384.csv
PYBIN="${PYBIN:-python3}"
SCRIPT=$PATTERN_DIR/kernels/test_join_quant_2d.py
...
DIMS=(64 128 256)
SIZES=(16384)
ITERS=50    # halved vs 8192 sweep -- each launch is ~4x heavier
...
run_one() {
  ...
  local cache="/tmp/triton_cache_join_16384_${pass,,}"
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_join_16384_${PASS,,}"
  mkdir -p "/tmp/triton_cache_join_16384_${PASS,,}"
