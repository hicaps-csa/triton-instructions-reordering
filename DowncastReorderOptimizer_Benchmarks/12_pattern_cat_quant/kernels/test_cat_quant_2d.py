"""2-D cat-quant benchmark.

Loads two M*N fp32 matrices A, B as 2-D tiles, concatenates them, and
truncates to int8. Tile shape is parameterized by BLOCK_M and BLOCK_N as
independent constexprs, so every (m, n) compiles to a unique kernel and
the sweep produces 16 distinct data points instead of collapsing to 7.
...
tt.cat -> arith.fptosi pattern that DowncastReorderOptimizer targets.

CLI arguments mirror test_cat_quant.py so the existing sweep harness
...
def run_benchmark_nsys(M, N, block_m, block_n, num_iterations):
    ...
    for _ in range(3):
        cat_quant_2d_kernel[grid](src_a, src_b, dst, M, N,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="2-D cat->quant Triton nsys benchmark")
