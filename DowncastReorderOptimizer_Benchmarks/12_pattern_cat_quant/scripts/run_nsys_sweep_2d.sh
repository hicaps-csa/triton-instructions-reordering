#!/bin/bash
# nsys sweep: 2-D cat-quant kernel
...
run_one() {
  ...
  local cache="/tmp/triton_cache_2d_${pass,,}"
  ...
  row=$(nsys stats --report cuda_gpu_kern_sum --format csv "$rep" 2>/dev/null \
        | grep cat_quant_2d_kernel | head -1)
  if [ -z "$row" ]; then
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_2d_${PASS,,}"
  mkdir -p "/tmp/triton_cache_2d_${PASS,,}"
