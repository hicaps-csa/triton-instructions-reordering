...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D cat_quant_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer (push fp32->i8 truncation before tt.cat)\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Bytes/launch: 4*M*N (read a) + 4*M*N (read b) + 1*M*2*N (write i8)\n")
        w("              = 10*M*N\n\n")
        ...
        if speedups:
            ...
            # Asymmetry detector: for each tile-product, are different (m, n)
            # splits showing different speedups? That would mean BLOCK_M and
            # BLOCK_N actually matter independently (not just the product).
        ...
        w("-" * 96 + "\n")
        if speedups:
            # Pick representative configs across the regimes for the larger size
