...
def bw_gb_s(M: int, N: int, ns: float) -> float:
    """Bytes/launch: 4*M*N (read a) + 1*M*N (write i8) = 5*M*N."""
    if ns <= 0:
        return 0.0
    return (5 * M * N) / (ns * 1e-9) / 1e9
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D reshape_quant_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer (push fp32->i8 truncation before tt.reshape)\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Bytes/launch: 4*M*N (read a) + 1*M*N (write i8) = 5*M*N\n\n")
        ...
        for size in sizes:
            ...
            w("  Pass ON  -- effective bandwidth (GB/s, 5*M*N bytes/launch)\n")
            render_matrix(f, DIMS, lambda m, n: (
