"""Aggregate ncu CSVs from the 2-D dual-quant sweep into a focused report."""
import csv
from pathlib import Path

RUN_DIR = Path("/tmp/ncu_runs_dual_quant")
...
def main():
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  ncu detailed profiling -- 2-D dual_quant_2d_kernel @ M = N = 8192\n")
        w("  4 configs (neutral / regr / win / big) x 2 pass states (one launch profiled per run)\n")
        w("  Diamond head : tt.cat (1-D fp32) -> { fptosi -> i8 store, truncf -> f16 store }\n")
        w("  Both users size-reducing -> dedup cache yields TWO rewritten chains; original cat dies.\n")
        w("=" * 96 + "\n\n")
