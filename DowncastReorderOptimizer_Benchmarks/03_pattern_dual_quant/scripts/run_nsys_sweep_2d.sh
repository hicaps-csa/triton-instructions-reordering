#!/bin/bash
# nsys sweep: 2-D dual-precision-quantize kernel
#   - 2 matrix sizes : 4096x4096 and 8192x8192
#   - 9 (m, n) tiles : {64, 128, 256} on each axis
#   - 2 pass states  : OFF, ON
# = 36 captures. The kernel exercises a diamond at the cat output where BOTH
# users are size-reducing (fptosi -> i8 and truncf -> f16). The dedup cache
# keyed on (OperationName, Type) gives two rewritten chains; the original
# fp32 cat dies (no non-reducer users left).
...
RESULTS_DIR=/tmp/nsys_runs_dual_quant
...
run_one() {
  ...
  local cache="/tmp/triton_cache_dualquant_${pass,,}"
  ...
  row=$(nsys stats --report cuda_gpu_kern_sum --format csv "$rep" 2>/dev/null \
        | grep dual_quant_2d_kernel | head -1)
  if [ -z "$row" ]; then
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_dualquant_${PASS,,}"
  mkdir -p "/tmp/triton_cache_dualquant_${PASS,,}"
