#!/bin/bash
# ncu detailed profiling for 2-D cat-trunc kernel.
...
PYBIN="${PYBIN:-python3}"
SCRIPT=$PATTERN_DIR/kernels/test_cat_trunc_2d.py
RUN_DIR=/tmp/ncu_runs_cat_trunc_2d
TRITON_DEFAULT_CACHE="$HOME/.triton/cache"

mkdir -p "$RUN_DIR"

# Initialized from the cat_quant_2d defaults; if the cat_trunc nsys sweep
# moves the win/regr to a different (m, n), update here before re-running.
CONFIGS=(
  "neutral 64  64"     # tile = 4096   (no spill either way)
  "mid     128 128"    # tile = 16384  (cat_quant_2d's ~1.10x; just above noise)
  "win     128 256"    # tile = 32768  (cat_quant_2d's 7.09x case)
  "big     256 256"    # tile = 65536  (cat_quant_2d's 1.29x case)
)
M=8192
N=8192
ITERS=10
...
set_pass "ON"
echo "DONE. /tmp/ncu_runs_cat_trunc_2d"
