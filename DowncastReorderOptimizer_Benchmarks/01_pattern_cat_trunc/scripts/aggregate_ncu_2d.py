"""Aggregate ncu CSVs from the 2-D cat-trunc sweep into a focused report."""
import csv
import os
from pathlib import Path

HERE = Path(os.path.dirname(os.path.abspath(__file__)))
RUN_DIR = Path("/tmp/ncu_runs_cat_trunc_2d")
TXT_PATH = HERE / "ncu_results_2d.txt"

# Default config set; the runner script's CONFIGS should match. If the
# nsys sweep moves the win/regr to a different (m, n), update both.
CONFIGS = [
    ("neutral", 64, 64),    # tile = 4096
    ("mid",     128, 128),  # tile = 16384
...
def main():
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  ncu detailed profiling -- 2-D cat_trunc_2d_kernel @ M = N = 8192\n")
        w("  4 configs (neutral / mid / win / big) x 2 pass states (one launch profiled per run)\n")
        w("=" * 96 + "\n\n")
        ...
        w("    neutral -> below register-spill cliff (expected: pass-OFF == pass-ON)\n")
        w("    mid     -> just above neutral; sweep showed ~1.10x in cat_quant\n")
        w("    win     -> headline speedup case from nsys sweep\n")
        w("    big     -> larger tile (potential regression / partial win)\n\n")
            ...
            for ps in PASSES:
                ...
                def cell(v, w_, fp=2):
                    if v is None:
                        return f"{'n/a':>{w_}}"
                    if v >= 1e6:
                        return f"{v/1e6:>{w_-1}.1f}M"
                    if v >= 1e3:
                        return f"{v/1e3:>{w_-1}.1f}K"
                    return f"{v:>{w_}.{fp}f}"
