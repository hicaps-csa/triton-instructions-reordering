"""Aggregate ncu CSVs from the 2-D broadcast-trunc sweep into a focused report."""
import csv
import os
from pathlib import Path

HERE = Path(os.path.dirname(os.path.abspath(__file__)))
RUN_DIR = Path("/tmp/ncu_runs_broadcast_trunc_2d")
TXT_PATH = HERE / "ncu_results_2d.txt"

# Default config set; the runner script's CONFIGS should match. Update both
# in lockstep when the nsys sweep moves the win/regr to a different (m, n).
CONFIGS = [
...
def main():
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  ncu detailed profiling -- 2-D broadcast_trunc_2d_kernel @ M = N = 8192\n")
