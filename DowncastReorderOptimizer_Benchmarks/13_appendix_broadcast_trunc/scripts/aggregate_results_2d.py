...
BW) so the (m, n) split is visible at a glance.

broadcast_trunc_2d variant: per launch we read N fp32 (the source row,
4*N bytes) and write M*N fp16 (2*M*N bytes). With M = N the read is a
~1/M tax on the write-dominated bandwidth, so the BW number is essentially
the destination write rate.
"""
import csv
import os
...
def bw_gb_s(M: int, N: int, ns: float) -> float:
    if ns <= 0:
        return 0.0
    bytes_per_launch = 4 * N + 2 * M * N
    return bytes_per_launch / (ns * 1e-9) / 1e9
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D broadcast_trunc_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer (push fp32->fp16 truncf above tt.broadcast)\n")
        w("=" * 96 + "\n\n")
        ...
        w("  Op chain    : broadcast(fp32 row) -> truncf -> fp16 [M, N]\n")
        w("                DowncastReorderOptimizer sinks the truncf so the broadcast\n")
        w("                emits fp16 directly.\n")
        w("  Bytes/launch: 4*N (read row, fp32) + 2*M*N (write tile, fp16)\n\n")

        for size in sizes:
            n_elems = size * size
            w(f"=== Matrix size: {size} x {size}  ({n_elems:,} elements/dst) ===\n\n")
            ...
            w("  Pass ON  -- effective bandwidth (GB/s, 4*N + 2*M*N bytes/launch)\n")
            render_matrix(f, DIMS, lambda m, n: (
        ...
        if speedups:
            ...
            w("  show the same or different timing? broadcast direction is N\n")
            w("  so wider-N tiles share more of the source row.)\n")
        ...
        w("  rather than two 2-D fp32 tiles. The pre-pass register footprint\n")
        w("  for the broadcast result is M*N fp32 (vs cat's 2*M*N fp32), so\n")
        w("  the spill cliff should sit at a wider tile here. Compare the\n")
        w("  speedup matrix against cat_trunc_2d/nsys_results_2d.txt.\n\n")
