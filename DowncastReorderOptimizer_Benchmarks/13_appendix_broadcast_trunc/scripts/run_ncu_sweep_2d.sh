#!/bin/bash
# ncu detailed profiling for 2-D broadcast-trunc kernel.
...
PYBIN="${PYBIN:-python3}"
SCRIPT=$PATTERN_DIR/kernels/test_broadcast_trunc_2d.py
RUN_DIR=/tmp/ncu_runs_broadcast_trunc_2d
TRITON_DEFAULT_CACHE="$HOME/.triton/cache"

mkdir -p "$RUN_DIR"

# Initialized from cat_trunc_2d defaults; once the broadcast_trunc nsys
# sweep finishes, update the win/big tuples to the ones aggregate_results
# recommends, then re-run this script.
CONFIGS=(
  "neutral 64  64"     # tile = 4096   (no spill expected)
  "win     128 256"    # tile = 32768  (cat_trunc_2d's headline win)
  "big     256 256"    # tile = 65536  (cat_trunc_2d's largest tile)
)
M=8192
N=8192
ITERS=10
...
set_pass "ON"
echo "DONE. /tmp/ncu_runs_broadcast_trunc_2d"
