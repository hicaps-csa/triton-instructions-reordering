#!/bin/bash
# nsys sweep: 2-D asymmetric-diamond-trans kernel
#   - 2 matrix sizes : 4096x4096 and 8192x8192
#   - 9 (m, n) tiles : {64, 128, 256} on each axis
#   - 2 pass states  : OFF, ON
# = 36 captures.
#
# Note: the current DowncastReorderOptimizer bails on ANY producer with a non-reducer
# user (strict gate, see DowncastReorderOptimizer.cpp:327-341). For this diamond
# (trans -> {truncf-fp16, mulf-fp32}) the OFF and ON IR are identical, so
# the perf numbers are expected to be within noise -- this sweep documents
# the gate working as intended on a fresh mover. To quantify the
# regression that would occur without the gate, see run_relaxed_sweep.sh.
...
run_one() {
  ...
  local cache="/tmp/triton_cache_adt_${pass,,}"
...
for PASS in OFF ON; do
  ...
  echo "=================================================================="
  set_pass "$PASS"
  rm -rf "/tmp/triton_cache_adt_${PASS,,}"
  mkdir -p "/tmp/triton_cache_adt_${PASS,,}"
