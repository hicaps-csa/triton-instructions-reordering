"""Turn nsys_results_2d.csv (asymmetric_diamond_trans_2d sweep) into a human-readable report.

For each matrix size, render 3x3 m-by-n tables (OFF us / ON us / speedup /
BW) and a per-tile detailed table. The kernel here is the asymmetric
diamond on tt.trans: trans (fp32) feeds both truncf (fp16 store) and
mulf 0.5 (fp32 store retained wide).
"""
import csv
import os
...
def write_report(rows):
    ...
    with open(TXT_PATH, "w") as f:
        w = f.write
        w("=" * 96 + "\n")
        w("  Triton 2-D asymmetric_diamond_trans_2d_kernel -- nsys timing sweep\n")
        w("  Pass under test: DowncastReorderOptimizer diamond gate (strict bail on non-reducer user)\n")
        w("  Diamond head : tt.trans (fp32) -> { truncf -> fp16 store, mulf 0.5 -> fp32 store }\n")
