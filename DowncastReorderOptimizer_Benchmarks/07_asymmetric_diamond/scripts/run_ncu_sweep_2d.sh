#!/bin/bash
# ncu detailed profiling for 2-D asymmetric_diamond_trans kernel.
# 4 configs (neutral / mid / win / big) x 2 pass states = 8 captures, all at
# M = N = 8192. With the current strict diamond gate, OFF and ON IR are
# identical -- this captures both states for completeness, expected to be
# within noise. For the relaxed-gate variant that quantifies the avoided
# regression, see run_relaxed_sweep.sh.
